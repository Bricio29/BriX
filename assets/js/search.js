...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (query) {
    ...
    searchContainer.innerHTML = `<p style="color: red; text-align: center; margin-bottom: 8px">Nada para mostrar...</p>`;
    searchHeading.innerText = 'Por favor, pesquise algo...';
  }
});
...
function fetchSearchResult(param, idsToToggle) {
  ...
  searchHeading.innerText = `Resultados da pesquisa para "${param}"`;
  ...
  fetch(url, options)
    ...
    .then((data) => {
      ...
      if (coins.length === 0) {
        coinsList.innerHTML =
          '<p style="color: red; text-align: center;">Nenhum resultado encontrado para moedas.</p>';
      }

      if (exchanges.length === 0) {
        exchangesList.innerHTML =
          '<p style="color: red; text-align: center;">Nenhum resultado encontrado para trocas.</p>';
      }

      if (nfts.length === 0) {
        nftsList.innerHTML =
          '<p style="color: red; text-align: center;">Nenhum resultado encontrado para nfts.</p>';
      }
    })
    .catch((error) => {
      idsToToggle.forEach((id) => {
...
function coinsResult(coins) {
  ...
  const table = createTable(['Rank', 'Moeda']);
...
function exchangesResult(exchanges) {
  ...
  const table = createTable(['Intercâmbio', 'Mercado']);
...
function nftsResult(nfts) {
  ...
  const table = createTable(['NFT', 'Símbolo']);
